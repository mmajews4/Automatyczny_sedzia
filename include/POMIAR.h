...
int blad_krancowki(){
  lcd.setCursor(1, 0); 
  lcd.print("Blad krancowki");//Zawór otwarty
...
int zliczanie_czasu(typ_wynik* temp_wynik){
  ...
  lcd.setCursor(5, 0); 
  lcd.print("Gotowy");//Moższ pić

  while(!krancowka_on()){            //  -----   Czekamy na otwarcie zaworu
    if(esc_flag)
    {
      lcd.clear();
      esc_flag = 0;
  ...
  if(krancowka_on()){              //  -----   Gdy otworzy sie zawór
    prevTime = millis();
    lcd.clear();
                  
    while(krancowka_on())          //  -----   Wyswietlanie uplywajacego czasu
    {
      currTime = millis();
