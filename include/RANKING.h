...
void ruch_menu_wynikow(char *nazwa_gracza, uint16_t *wynik_1, uint16_t *wynik_2){
  ...
  if(up_flag){                        // 1 do góry
    ...
    while(strcmp(nazwa_gracza, wyniki[i_1].nazwa) != 0 && (i_1 > 1)){
      i_1--;                          // Wyszykanie wcześniejszego wyniku danego gracza
    }

    if(strcmp(nazwa_gracza, wyniki[i_1].nazwa) == 0)
  ...
  if(down_flag){                      // 1 do dołu                                // Sprawdzić co się stanie jak wyjdę poza miejsce 100
    ...
    while(strcmp(nazwa_gracza, wyniki[i_2].nazwa) != 0 && (i_2 <= wyniki[0].czas)){  
  ...
  if(left_flag){                      // zerowanie flag żeby nie ranking się nie przesuwał po wyjściu z menu gracza
    left_flag = 0;
  }
  if(right_flag){                                             
    right_flag = 0;
...
void menu_wynikow_gracza(uint16_t miejsce){

  uint16_t wynik_1, wynik_2;
  uint16_t i = 1;
  float czas = wyniki[ranking_index[miejsce]].czas;
  char nazwa_gracza[21];
  strcpy(nazwa_gracza, wyniki[ranking_index[miejsce]].nazwa);

  while((czas != wyniki[i].czas) && strcmp(nazwa_gracza, wyniki[i].nazwa)){    // Przejście do pierwszego wyniku gracza
    i++;
  }
  wynik_1 = i;                                                                    // Zapisanie pozycji pierwszego wyniku
  i++;                                                                            // Wyjście dalej za pierwszy wynik żeby go nie wzieło jako drugi

  while(strcmp(nazwa_gracza, wyniki[i].nazwa) && (i <= wyniki[0].czas)){       // Przejście do drugiego wyniku gracza
    i++;
  }
  wynik_2 = i;
  ...
  while(1)
  {
    ...
    Serial.print("Nazwa_gracza    ");
    Serial.println(nazwa_gracza);
    Serial.println("    ");
    ...
    ruch_menu_wynikow(nazwa_gracza, &wynik_1, &wynik_2);            // Funkcja do przemieszczania się po rankingu
  }
}
